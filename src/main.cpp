...
int main() {
    ...
    EngineSoundGenerator engineSoung(mainSamples, engineDef, 1000.0f, 0.5f);
    ...
    AudioContext engineCtx("enGeneration Program + Librarygine", {&engineSound});
    ...
    Biquad torqueFilter(bq_type_lowpass, 5.0f / 100.0f, 0.707f, 0.0f);
    ...
    Damper gasAvg(0.5);       // Smooth out gas inputs
...
engineCtx.addFilter(makeBiquad(bq_type_peak, 2600.0f, 4.36f, 4.0f));
...
// context.addFilter(std::make_unique<ForceSilenceFilter>());
    ...
    while (window.isOpen()) {
        ...
        engineSound.setRPM(carRpm);
        engineSound.setAmplitude(std::clamp(carTorque / 350, 0.0f, 1.0f));
